//Utilizar la funcion fopen(), para determinar si existe un archivo de texto (.txt)
//fopen(nombre de archivo, modo);
//nombre de archivo = cadena... contiene el identificador externo del arhcivo
//modo = cadena             ... contiene el modo en que se va ser tratado el archivo

#include<stdio.h> //Libreria de entrada y salida
#include<stdlib.h> //Libreria estandar

FILE *fd; //Apuntador a archivo

int main(){
	
	//Declaracion de variable
	char direccion[] = "C:\\Users\\river\\OneDrive\\Documents\\Curso de C\\Ejercicios de Archivos\\texto.txt";
	
	//El apuntador al archivo lo igualamos a fopenn
	fd = fopen(direccion, "r"); //r = leer(read)
	
	if(fd == NULL){//Condicion de fallo
		printf("No se ha encontrado el lugar para crear el archivo");
	}else{ //De lo contrario imprime la direccion de ubicacion
		printf("Archivo se ha encontrao");
		printf("\nUbicacion: %s", direccion);
	}
	
	fclose(fd); //Cerramos el archivo pase lo que pase
	
	return 0; //Se termina el programa
	
} 
